...
def show_wallet_window(profile_data):
    ...
    wallet_window.title("Swamp Light Wallet")
    wallet_window.config(bg=bg)

    logo_small = tk.PhotoImage(file="sample/swamp_light_400.png")
    window_width = 450
    window_height = 650
    ...
    def update_balance():
        ...
        balance_label.config(text=f"Address Balance: {round(balance, 4)} SWAMP")
        ...
        def prepare_tx():
            ...
            if wallet.verify_address(receiver):
                ...
                if UTXO_RPC.calculate_total_balance(selected_sender_address) <= amount:
                    ...
                    messagebox.showerror(title="Error", message="Not enough SWAMP!")
                    return
                else:
                ...
                print(f"\nError! Receiver address is not a valid SWAMP address!")
                messagebox.showerror(title="Error", message="Receiver address is not a valid SWAMP address!")
                return
    ...
    send_swamp_button = tk.Button(wallet_window, text="Send SWAMP", pady=4, padx=0, width=28, command=send_swamp,
                                 bg=bg,
                                 fg=fg,  # Text color
                                 borderwidth=1,  # Border width
                                 highlightthickness=1,  # Highlight thickness for focus
    ...
    send_swamp_button.pack(side="top")
    ...
    console = scrolledtext.ScrolledText(wallet_window, height=10)
    ...
    def update_status():
        ...
        wallet_window.after(30000, update_status)  # seconds delay
...
def show_start_window():
    start_window = tk.Tk()
    start_window.title("Swamp Light Wallet 1.0.1")
    logo = tk.PhotoImage(file="sample/swamp_light.png")
    window_width = 750
    window_height = 274
